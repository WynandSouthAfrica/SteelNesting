# Steel Nesting Planner v13.9 — Latin-1 safe text + PNG/JPG logo + PG Bison header (18 Aug 2025)
# Modes: Nest by Required Cuts · Nest from Stock · View Summary Report
# Changes from v13.8:
# - All PDF strings normalized to latin-1-safe ASCII via safe_text()
# - Removed em/en dashes; standardized to '-' to avoid latin-1 encode errors
# - Retains: real PNG plot files (no BytesIO), JPG/PNG logos, PG Bison header, section-based PDF
...
st.set_page_config(page_title="Steel Nesting Planner v13.9", layout="wide")
st.title("🧰 Steel Nesting Planner v13.9 — PG Bison Header + Section-Based PDF")

KERF_DEFAULT_MM = 2.0
STOCK_DEFAULT_MM = 6000
DEFAULT_LOGO_B64 = ""  # optional embedded default logo as base64
...
# Replace smart punctuation/non-breaking space with ascii that latin-1 can handle
_REPL = {
    "\u2014": "-",  # em dash —
    "\u2013": "-",  # en dash –
    "\u2012": "-",  # figure dash
...
def safe_text(val) -> str:
    ...
    try:
        ...
        # Last resort: replace unencodable chars with '?'
...
def bars_to_text_lines(bars: List[Dict], stock_len_mm: int) -> List[str]:
    lines = [f"Stock {stock_len_mm} mm - Bars used: {len(bars)}"]  # ASCII hyphen
...
def save_bars_plot_png(bars: List[Dict], stock_len_mm: int, kerf_mm: float) -> str:
    ...
    rows = len(bars); height = max(2.0, 0.35 * rows + 1.0)
    fig, ax = plt.subplots(figsize=(9, height))
    ax.set_xlim(0, stock_len_mm); ax.set_ylim(0, rows)
    ax.set_xlabel("mm"); ax.set_ylabel("Stock Bars")
...
if mode == "Nest from Stock":
    ...
    example_stock = pd.DataFrame({"Section Size": ["152x152x23 kg/m", "100x50 RSC Channel"], "Stock Length (mm)": [6000, 6000], "Bars Available": [3,2]})
...
def draw_meta_table(pdf: FPDF, meta: Dict):
    ...
    for label in ["Project","Location","Drawing Number","Revision","Material","Cutting List By","Date Created"]:
        pdf.set_x(left_margin)
        pdf.set_font("Helvetica", "", 11); pdf.cell(col_label_w, row_h, safe_text(label), border=1)
        pdf.set_font("Helvetica", "B", 11); pdf.cell(col_value_w, row_h, safe_text(meta.get(label,"")), border=1, ln=1)
    pdf.ln(2)

def write_section_block(pdf: FPDF, section: str, stock_len_mm: int, kerf_mm: float, df_section: pd.DataFrame, bars: List[Dict]):
    pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 7, safe_text(f"Section Size   {section}"), ln=1, border=1); pdf.ln(1)
    ...
    pdf.cell(45, 7, safe_text("Cut Length (mm)"), border=1)
    pdf.cell(25, 7, safe_text("Qty"), border=1)
    pdf.cell(40, 7, safe_text("Stock Len (mm)"), border=1)
    pdf.cell(80, 7, safe_text("Note"), border=1, ln=1)

    pdf.set_font("Helvetica", "", 10)
    for _, r in df_section.iterrows():
        pdf.cell(45, 7, safe_text(clean_int(r["Cut Length (mm)"])), border=1)
        pdf.cell(25, 7, safe_text(clean_int(r["Quantity"])), border=1)
        s_override = clean_int(r.get("Stock Length (mm)", 0), 0)
        pdf.cell(40, 7, safe_text(s_override if s_override>0 else stock_len_mm), border=1)
        pdf.cell(80, 7, safe_text(str(r.get("Note",""))[:45]), border=1, ln=1)
    pdf.ln(1)

    # Real PNG plot
    img_path = save_bars_plot_png(bars, stock_len_mm, kerf_mm)
    pdf.set_font("Helvetica", "B", 10); pdf.cell(0, 6, safe_text("Cut Layout Visualization:"), ln=1)
    pdf.image(img_path, w=190); pdf.ln(4)

def consolidated_pdf(meta: Dict, logo_path: str, payloads: List[Tuple[str, int, float, pd.DataFrame, List[Dict]]]) -> bytes:
    pdf = FPDF(orientation="P", unit="mm", format="A4"); pdf.set_auto_page_break(auto=True, margin=10)
    for idx, (section, stock_len_mm, kerf_mm, df_section, bars) in enumerate(payloads):
        pdf.add_page(); draw_header_with_logo(pdf, logo_path); draw_meta_table(pdf, meta)
...
def payloads_from_stock(req_df: pd.DataFrame, stock_df: pd.DataFrame, kerf_mm: float):
    ...
    for section, g in req_groups.items():
        ...
        dominant_len = (inv[0][0] if len(inv)==0 else int(pd.Series([b["len"] for b in bars]).mode().iloc[0])) if len(bars)>0 else (inv[0][0] if len(inv)>0 else 6000)
        normalized = []
        for b in bars:
...
with col_run: run = st.button("⚙️ Run Nesting & Build PDF", type="primary")
...
if run:
    ...
    try:
        ...
        # On-screen summary
        if mode in ("Nest by Required Cuts", "Nest from Stock") and len(payloads) > 0:
            st.write("---"); st.subheader("📊 Quick On-Screen Summary (per Section)")
            rows = []
            for section, slen, k, g, bars in payloads:
